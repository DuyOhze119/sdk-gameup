    ...
    public class GameUpSetupWindow : EditorWindow
    {
        ...
        private void SaveAllData()
        {
            foreach (var tab in _allTabs)
            {
                ...
                {
                    Debug.LogError($"[GameUpSDK] Lỗi khi lưu {tab.Title}: {e.Message}");
                }
            }

            AssetDatabase.SaveAssets();
            Debug.Log("[GameUpSDK] Đã lưu cấu hình thành công vào tất cả các Prefab/Asset độc lập!");
        }
        ...
        private static void FixNestedPrefabReferences(string sdkPrefabPath)
        {
            ...
            if (changed)
            {
                ...
                Debug.Log("[GameUpSDK] Đã cập nhật lại link các Prefab con trong SDK.prefab trỏ đúng về thư mục Assets/SDK/Prefabs.");
            }
