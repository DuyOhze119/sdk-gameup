...
namespace GameUpSDK
{
    ...
    {
        private const string LogTag = "[GameUpSDK][AppMetrica]";
        ...
        {
            if (!ShouldPrintDebug()) return;
            Debug.LogWarning($"{LogTag} Skip {method} ({eventName}): {reason}");
        }
        ...
        private static void DebugLogEditorStub(string method, string eventName, Dictionary<string, string> parameters)
        {
            ...
            Debug.Log($"{LogTag} Editor stub {method}: {eventName} | {detail} (chỉ gửi lên server trên build Android/iOS)");
        }
#endif
        ...
        {
            if (!ShouldPrintDebug()) return;
            Debug.LogWarning($"{LogTag} Skip {method} ({eventName}): APPMETRICA_DEPENDENCIES_INSTALLED không bật");
        }
    }
}
